	...
	for i := 0; i < 64; i++ {
		if board.White.All >> i % 2 == 1 {
			if board.White.Pawns >> i % 2 == 1 {
				earlyScore += earlyPawns[i]
				lateScore += latePawns[i]
			} else if board.White.Knights >> i % 2 == 1 {
				earlyScore += earlyKnights[i]
				lateScore += lateKnights[i]
				phase += 1
			} else if board.White.Bishops >> i % 2 == 1 {
				earlyScore += earlyBishops[i]
				lateScore += lateBishops[i]
				phase += 1
			} else if board.White.Rooks >> i % 2 == 1 {
				earlyScore += earlyRooks[i]
				lateScore += lateRooks[i]
				phase += 2
			} else if board.White.Queens >> i % 2 == 1 {
				earlyScore += earlyQueens[i]
				lateScore += lateQueens[i]
				phase += 4
			} else if board.White.Kings >> i % 2 == 1 {
				earlyScore += earlyKings[i]
				lateScore += lateKings[i]
			}
		} else if board.Black.All >> i % 2 == 1 {
			j := i ^ 56
			if board.Black.Pawns >> i % 2 == 1 {
				earlyScore -= earlyPawns[j]
				lateScore -= latePawns[j]
			} else if board.Black.Knights >> i % 2 == 1 {
				earlyScore -= earlyKnights[j]
				lateScore -= lateKnights[j]
				phase += 1
			} else if board.Black.Bishops >> i % 2 == 1 {
				earlyScore -= earlyBishops[j]
				lateScore -= lateBishops[j]
				phase += 1
			} else if board.Black.Rooks >> i % 2 == 1 {
				earlyScore -= earlyRooks[j]
				lateScore -= lateRooks[j]
				phase += 2
			} else if board.Black.Queens >> i % 2 == 1 {
				earlyScore -= earlyQueens[j]
				lateScore -= lateQueens[j]
				phase += 4
			} else if board.Black.Kings >> i % 2 == 1 {
				earlyScore -= earlyKings[j]
				lateScore -= lateKings[j]
			}
		}
	}	
